(() => {
  ...
  async function runSearch() {
    ...
    try {
      ...
      results.innerHTML = found.map(({ item }) => `<a class="result" href="${escape(item.url)}"><small>${escape(item.course)} · ${escape(item.module)} · ${escape(item.type)}</small><h2>${escape(item.title)}</h2><p>${excerpt(item.text, term)}</p></a>`).join('');
